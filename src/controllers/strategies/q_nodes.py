...
class QNodes(SIA):
    ...
    def algorithm(self, vertices: list[tuple[int, int]]):
        ...
        vertices_fase = list(
            vertices
        )  # se hace una copia de todos los vertices para controlar el ciclo
        ...
        for i in range(total):
            ...
            for j in range(len(deltas_ciclo) - 1):
                ...
                if rank == 0:
                    ...
                    chunks = [
                        args_list[k * chunk_size : (k + 1) * chunk_size]
                        for k in range(size)
                    ]
                else:
                    chunks = None
                ...
                if rank == 0:
                    ...
                    emd_local = float("inf")
                    indice_mip = None
                    for k, (emd_union, emd_delta, dist_marginal_delta) in enumerate(
                        resultados
                    ):
            ...
            if rank == 0:
                ...
                self.memoria_particiones[tuple(clave)] = (
